import 'package:flutter/widgets.dart';
import '../../base_icon.dart';

/// Monochrome style HouseBoltAlt icon.
///
/// Example:
/// ```dart
/// MonochromeHouseBoltAlt(size: 24, color: Colors.blue);
/// ```
class MonochromeHouseBoltAlt extends StatelessWidget {
  /// Icon size (width & height).
  final double size;

  /// Icon color.
  final Color? color;

  /// Creates a MonochromeHouseBoltAlt.
  const MonochromeHouseBoltAlt({
    super.key,
    this.size = 24,
    this.color,
  });

  @override
  Widget build(BuildContext context) {
    return NasIcon(
      svg: '''
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill="currentColor" fill-rule="evenodd" d="M13.3 2.24a5 5 0 0 0-2.6 0c-.98.26-1.83.93-3.07 1.9L4.75 6.36a9 9 0 0 0-1.77 1.61 5 5 0 0 0-.8 1.63C2 10.27 2 10.98 2 11.99v2.65q-.01 1.62.06 2.67c.06.73.18 1.37.49 1.96a5 5 0 0 0 2.18 2.18c.6.3 1.23.43 1.96.5q1.04.06 2.67.05h5.28q1.62.02 2.67-.06a5 5 0 0 0 1.96-.49 5 5 0 0 0 2.19-2.18c.3-.6.42-1.23.48-1.96q.07-1.05.06-2.67V12c0-1.01 0-1.72-.18-2.38a5 5 0 0 0-.8-1.63 9 9 0 0 0-1.77-1.6l-2.88-2.25c-1.24-.96-2.09-1.63-3.06-1.89" clip-rule="evenodd" opacity=".3"/><path fill="currentColor" fill-rule="evenodd" d="M13.31 10.08a1 1 0 1 0-1.62-1.16l-2.5 3.5A1 1 0 0 0 10 14h2.06l-1.37 1.92a1 1 0 0 0 1.62 1.16l2.5-3.5A1 1 0 0 0 14 12h-2.06z" clip-rule="evenodd"/></svg>
''',
      size: size,
      color: color,
    );
  }
}
