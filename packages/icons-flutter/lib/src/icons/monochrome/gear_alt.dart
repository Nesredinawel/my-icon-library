import 'package:flutter/widgets.dart';
import '../../base_icon.dart';

/// Monochrome style GearAlt icon.
///
/// Example:
/// ```dart
/// MonochromeGearAlt(size: 24, color: Colors.blue);
/// ```
class MonochromeGearAlt extends StatelessWidget {
  /// Icon size (width & height).
  final double size;

  /// Icon color.
  final Color? color;

  /// Creates a MonochromeGearAlt.
  const MonochromeGearAlt({
    super.key,
    this.size = 24,
    this.color,
  });

  @override
  Widget build(BuildContext context) {
    return NasIcon(
      svg: '''
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill="currentColor" fill-rule="evenodd" d="M20 12q0-.86-.18-1.68l1.08-1.89a1 1 0 0 0 .02-.95 10 10 0 0 0-1.22-1.85 1 1 0 0 0-.88-.36l-2.13.24a8 8 0 0 0-1.82-.98L13.9 2.6a1 1 0 0 0-.78-.54 10 10 0 0 0-2.22 0 1 1 0 0 0-.78.54l-.98 1.93q-.98.38-1.8.98l-2.16-.24a1 1 0 0 0-.88.36 10 10 0 0 0-1.23 1.85 1 1 0 0 0 .02.95l1.1 1.91a8 8 0 0 0-.02 3.29l-1.08 1.9a1 1 0 0 0-.02.95 10 10 0 0 0 1.22 1.85 1 1 0 0 0 .89.36l2.1-.24q.85.63 1.85 1.02l.99 1.93c.15.3.45.5.78.54a10 10 0 0 0 2.22 0 1 1 0 0 0 .78-.54l.98-1.93q1.01-.39 1.86-1.02l2.09.24a1 1 0 0 0 .88-.36 10 10 0 0 0 1.22-1.85 1 1 0 0 0-.02-.95l-1.07-1.88q.17-.8.17-1.65m-8 4a4 4 0 1 0 0-8 4 4 0 0 0 0 8" clip-rule="evenodd" opacity=".3"/><circle cx="12" cy="12" r="4" fill="currentColor"/></svg>
''',
      size: size,
      color: color,
    );
  }
}
