import 'package:flutter/widgets.dart';
import '../../base_icon.dart';

/// Monochrome style CalendarUser icon.
///
/// Example:
/// ```dart
/// MonochromeCalendarUser(size: 24, color: Colors.blue);
/// ```
class MonochromeCalendarUser extends StatelessWidget {
  /// Icon size (width & height).
  final double size;

  /// Icon color.
  final Color? color;

  /// Creates a MonochromeCalendarUser.
  const MonochromeCalendarUser({
    super.key,
    this.size = 24,
    this.color,
  });

  @override
  Widget build(BuildContext context) {
    return NasIcon(
      svg: '''
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill="currentColor" fill-rule="evenodd" d="M7 2a1 1 0 0 1 1 1v1h8V3a1 1 0 1 1 2 0v1q.69 0 1.2.03c.39.03.78.1 1.16.3a3 3 0 0 1 1.31 1.3c.2.39.27.78.3 1.17q.04.57.03 1.36v9.68q0 .8-.03 1.36c-.03.39-.1.78-.3 1.16a3 3 0 0 1-1.3 1.31c-.39.2-.78.27-1.18.3q-.55.04-1.35.03H6.16q-.8 0-1.36-.03a3 3 0 0 1-1.16-.3 3 3 0 0 1-1.31-1.3c-.2-.39-.27-.78-.3-1.18A18 18 0 0 1 2 17.85V8.16q0-.8.03-1.36c.03-.39.1-.78.3-1.16a3 3 0 0 1 1.3-1.31c.39-.2.78-.27 1.17-.3Q5.31 4 6 4V3a1 1 0 0 1 1-1" clip-rule="evenodd" opacity=".3"/><path fill="currentColor" fill-rule="evenodd" d="M14.98 10a3 3 0 1 1-6 0 3 3 0 0 1 6 0m2.43 7.22a6 6 0 0 0-10.63 0c-.2.4-.31.6-.27.91.03.23.22.55.41.69.26.18.56.18 1.15.18h8.05c.6 0 .9 0 1.16-.18.19-.14.38-.46.4-.69.05-.31-.05-.51-.27-.91" clip-rule="evenodd"/></svg>
''',
      size: size,
      color: color,
    );
  }
}
