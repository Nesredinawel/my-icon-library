import 'package:flutter/widgets.dart';
import '../../base_icon.dart';

/// Monochrome style MessageSquareCheck icon.
///
/// Example:
/// ```dart
/// MonochromeMessageSquareCheck(size: 24, color: Colors.blue);
/// ```
class MonochromeMessageSquareCheck extends StatelessWidget {
  /// Icon size (width & height).
  final double size;

  /// Icon color.
  final Color? color;

  /// Creates a MonochromeMessageSquareCheck.
  const MonochromeMessageSquareCheck({
    super.key,
    this.size = 24,
    this.color,
  });

  @override
  Widget build(BuildContext context) {
    return NasIcon(
      svg: '''
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"><path fill="currentColor" fill-rule="evenodd" d="M17.84 3H6.16q-.8 0-1.36.03c-.39.03-.78.1-1.16.3a3 3 0 0 0-1.31 1.3c-.2.39-.27.78-.3 1.17Q2 6.37 2 7.16v7.68q0 .8.03 1.36c.03.39.1.78.3 1.16a3 3 0 0 0 1.3 1.31c.39.2.78.27 1.17.3q.57.04 1.36.03h10.08l.43.01.18.04.38.18 3.32 1.66A1 1 0 0 0 22 20V7.16q0-.8-.03-1.36a3 3 0 0 0-.3-1.16 3 3 0 0 0-1.3-1.31c-.39-.2-.78-.27-1.18-.3A18 18 0 0 0 17.85 3M15.7 9.72a1 1 0 1 0-1.38-1.44l-3.46 3.33-1.16-1.1a1 1 0 0 0-1.38 1.43l1.84 1.78a1 1 0 0 0 1.39 0z" clip-rule="evenodd" opacity=".3"/><path fill="currentColor" d="M15.72 8.3a1 1 0 0 1-.03 1.42l-4.15 4a1 1 0 0 1-1.39 0l-1.84-1.78a1 1 0 1 1 1.38-1.44l1.16 1.11 3.46-3.33a1 1 0 0 1 1.41.03"/></svg>
''',
      size: size,
      color: color,
    );
  }
}
