import 'package:flutter/widgets.dart';
import '../../base_icon.dart';

/// Outline style Tent icon.
///
/// Example:
/// ```dart
/// OutlineTent(size: 24, color: Colors.blue);
/// ```
class OutlineTent extends StatelessWidget {
  /// Icon size (width & height).
  final double size;

  /// Icon color.
  final Color? color;

  /// Creates a OutlineTent.
  const OutlineTent({
    super.key,
    this.size = 24,
    this.color,
  });

  @override
  Widget build(BuildContext context) {
    return NasIcon(
      svg: '''
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" d="m12 4-6.7 6.26c-.3.27-.45.41-.56.57a2 2 0 0 0-.25.46c-.07.19-.1.38-.17.78l-1.01 6.07c-.1.64-.16.96-.07 1.2a1 1 0 0 0 .44.53c.23.13.56.13 1.2.13H12m0-16 6.7 6.26c.3.27.45.41.56.57q.16.21.25.46c.07.19.1.38.17.78l1 6.07c.12.64.17.96.08 1.2a1 1 0 0 1-.44.53c-.23.13-.56.13-1.2.13H17M12 4v8m5 8h-5m5 0-5-8m0 8v-8"/></svg>
''',
      size: size,
      color: color,
    );
  }
}
